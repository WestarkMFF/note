...
<script setup lang="ts">
  import { ref, reactive, watch, provide, computed, onUnmounted, useAttrs, Ref, readonly } from 'vue';
  ...
  const attrs = computed({
    get() {
      ...
      const result = Object.assign({}, tableRequiredParams.value, tableConfig.value, useAttrs());
      ...
      if (result.res) {
        ...
        !result.pagination ? (result.pagination = { total }) : (result.pagination.total = total);
      }
    ...
    return cols.map((col: Type.TableBoxColumn) => {
      if (typeof col === 'string') {
        ...
        if (shortcut.length == 1) {
          console.warn(`colItem [${col}] formate is wrong，string "$\{title\},$\{dataIndex\} expected"`);
          col = {};
        }
